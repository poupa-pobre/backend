...
- `apps/gastos` (Fase 2) tem `Gasto` + `GastoTag` (N:M com `Tag`) em `/api/gastos/`. `mes_referencia` derivado em `Gasto.save()` (competência do cartão no crédito via `competencia_de`, senão 1º dia do mês) e a fatura do mês é garantida via `fatura_do_mes()`. Serializer valida scoping (categoria/subcategoria/cartão/vínculo/tags do dono), RN-020 (cartão obrigatório só no crédito) e RN-021 (compartilhado exige vínculo aceito + `valor_dono+valor_vinculado=valor`; não-compartilhado zera o rateio). FKs: categoria/cartão `PROTECT` (soft delete), subcategoria/vínculo `SET_NULL` (RN-002). Filtros de listagem por `mes_referencia`/`categoria`/`cartao`/`forma_pagamento`. **Scanner de cupom / compra detalhada (RF-022..025, Fase 5)**: `CompraDetalhada` (1:1 com `Gasto`: estabelecimento/origem/`url_nfce`) + `ItemCompra` (nome/codigo/quantidade/unidade/valor_unitario/valor/categoria/`identificado`), gravados **aninhados** no `GastoSerializer` (`compra_detalhada` writable; `_salvar_compra` substitui no update; leitura segura via `get_attribute` que devolve `None` quando o OneToOne reverso não existe). A action `POST /gastos/parsear-cupom/` (`url_path="parsear-cupom"`, **não persiste**) chama `parser.parsear_cupom(texto_ocr, url_qr)`: parser **heurístico** do texto do OCR (formato NFC-e `cod desc qtd UN x unit total`, fallback p/ descrição+valor; itens sem qtd/unit vêm `identificado=False` p/ revisão) + leitura de chave/UF/url do QR (RN-024). Devolve preview (itens, total, total_confere, cnpj, chave, uf). **OCR é on-device no app (ML Kit); o backend só parseia o texto.**
...
Próximo na **Fase 5** (periféricos): `apps/notificacoes` (`ConfigNotificacao`, envio por email + tentativa de Google Calendar RF-090/091), `apps/importacao` (upload OFX/CSV, sugestão de categoria, detecção de duplicata RF-111/RN-110) e **export PDF dos relatórios** (RF-101/102). O **scanner de cupom** (`CompraDetalhada`+`ItemCompra`, NFC-e/OCR RF-022..025) já foi implementado em `gastos` (ver acima) — o app mobile já consumia o contrato. Refino possível do parser: data de emissão, sugestão de categoria por item, mais layouts de cupom.
